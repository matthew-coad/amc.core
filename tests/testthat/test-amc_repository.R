...
test_that("Datasource repository path is child path of amc repository", {
  ...
  test_repository_path <- prepare_local_test_repository(test_name)
  expected_datasource_path <- file.path(test_repository_path, test_dataset1$datasource_code)
...
test_that("Dataset repository path is child path of datasource repository", {
  ...
  test_repository_path <- prepare_local_test_repository(test_name)
  expected_dataset_path <- file.path(test_repository_path, test_datasource$datasource_code, test_dataset1$dataset_code)
  expect_equal(get_amc_dataset_repository_path(test_dataset1), expected_dataset_path)
})
...
test_that("Dataset path is child path of dataset repository", {
  ...
  test_repository_path <- prepare_local_test_repository(test_name)
  expected_dataset_path <- file.path(test_repository_path, test_datasource$datasource_code, test_dataset1$dataset_code, test_filename)
  expect_equal(get_amc_dataset_path(test_dataset1, test_filename), expected_dataset_path)
})
