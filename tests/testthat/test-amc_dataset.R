...
test_that("list_amc_dataset includes test dataset", {
  ...
  dataset <- datasets |> dplyr::filter(dataset_code == test_quick_dataset$dataset_code)
  expect_equal(dataset$dataset_code,  test_quick_dataset$dataset_code)
})

test_that("resolve_amc_dataset via character returns expected dataset", {
  dataset <- resolve_amc_dataset("test_quick")
  expect_equal(dataset$dataset_code, "test_quick")
})

test_that("resolve_amc_dataset using dataset just returns dataset", {
  dataset <- resolve_amc_dataset(test_quick_dataset)
  expect_equal(dataset$dataset_code, "test_quick")
})
...
test_that("prepare_amc_dataset_repository creates repository folder", {
  ...
  test_dataset_path <- get_amc_dataset_repository_path(test_quick_dataset)
  prepare_amc_dataset_repository(test_quick_dataset)
...
test_that("import_amc_dataset_file copies file to dataset", {
  ...
  expected_test_filepath <- get_amc_dataset_path(test_quick_dataset, test_filename)
  import_amc_dataset_file(test_quick_dataset, test_filepath)
...
#   test_filename <- "test_quick.csv"
#   expected_test_filepath <- get_amc_dataset_path(test_quick_dataset, test_filename)
#   import_test_data(test_quick_dataset, test_filename)
#   expect_true(file.exists(expected_test_filepath))
# })
...
test_that("download_amc_dataset invokes dataset download func", {
  ...
  test_datafile_path <- get_amc_dataset_path(test_quick_dataset, test_quick_filename)
  download_amc_dataset(test_quick_dataset)
...
#   download_amc_dataset(test_quick_dataset)
#   run1 <- read_amc_dataset_state(test_quick_dataset)
#   # Wait
#   Sys.sleep(0.1)
#   download_amc_dataset(test_quick_dataset)
#   run2 <- read_amc_dataset_state(test_quick_dataset)
#
...
#   test_datafile_path <- get_amc_dataset_path(test_quick_dataset, test_quick_filename)
#
#   state <- prepare_amc_dataset(test_quick_dataset)
...
#
#   state1 <- prepare_amc_dataset(test_quick_dataset)
#   # Wait to ensure time change
#   Sys.sleep(0.1)
#   state2 <- prepare_amc_dataset(test_quick_dataset)
#
