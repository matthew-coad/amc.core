test_that("read_amc_dataset_state returns missing if state not saved", {
  ...
  state <- read_amc_dataset_state(test_quick_dataset)
...
test_that("save_amc_dataset_state saves state", {
  ...
  saved_state <- save_amc_dataset_state(test_quick_dataset, amc_dataset_status$ready)
  state <- read_amc_dataset_state(test_quick_dataset)
  expect_equal(saved_state$status, amc_dataset_status$ready)

  expect_equal(state$datasource_code, test_quick_dataset$datasource_code)
  expect_equal(state$dataset_code, test_quick_dataset$dataset_code)
